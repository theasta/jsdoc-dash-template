...
var docletRecord = require('./docletRecord');
 ...
 * @returns {Object|*}
 */
TemplateRenderer.prototype.getCategory = function (category) {
...
TemplateRenderer.prototype.collectSourceFiles = function () {
  ...
  this.data().each(function(doclet) {
    ...
    var sourcePath;
    if (doclet.meta) {
      sourcePath = docletRecord.getPathFromDoclet(doclet);
  ...
  this.data().each(function(doclet) {
    ...
    var docletPath;
    if (doclet.meta) {
      docletPath = docletRecord.getPathFromDoclet(doclet);
  ...
  this.data().each(function(doclet) {
    ...
    if (doclet.see) {
      doclet.see.forEach(function(seeItem, i) {
        doclet.see[i] = docletRecord.hashToLink(doclet, seeItem);
      });
    }
  });
};
...
TemplateRenderer.prototype.addSignatureTypes = function (doclet) {
  var types = docletRecord.getSignatureTypes(doclet);
...
TemplateRenderer.prototype.addSignatureReturns = function (doclet) {
  ...
  var ret = docletRecord.getSignatureReturns(doclet);
...
TemplateRenderer.prototype.addSignatures = function () {
  ...
  this.data().each(function(doclet) {
    ...
    if ( docletRecord.needsSignature(doclet) ) {
