...
/**
 * Create the navigation object for the sidebar.
 ...
 * @return {Object} The data for the navigation sidebar.
 */
function buildNav(members) {
    ...
    var fn = function (key) {
        ...
        members[key].forEach(function (item) {
            ...
            if (!seen.hasOwnProperty(item.longname)) {
                arr.push({
                    url: url
                });
            }
    ...
    ['modules', 'classes', 'namespaces', 'mixins', 'interfaces', 'events'].forEach(function (key) {
        fn(key);
    });
    ...
    members.globals.forEach(function (item) {
        ...
        // next line diff
        if (item.kind !== 'typedef' && !seen.hasOwnProperty(item.longname)) {
            nav.globals.items.push({
                url: url
            });
        }
        seen[item.longname] = true;
    ...
    members.tutorials.forEach(function(item) {
        var url = tutoriallink(item.name);
        var entryPath = regExp.exec(url)[1];

        nav.tutorials.items.push({
            url: url
        });
    ...
    fs.writeFileSync(outdir + '/docset.json', JSON.stringify(entries), 'utf8');
